...
var xObject = JSON.parse(x); //声明一个hash数组来贮存添加得网页得logo和url
...
var $siteList = $(".siteList"); //获取最后一个li

var $lastLi = $(".lastLi"); //遍历hash数组得函数，并且把数组里面储存得网页放到最后一个添加按钮得前面

var render = function render() {
  //添加之前先把之前的网页删除
  $siteList.find("li:not(.lastLi)").remove();
  hashmap.forEach(function (node, index) {
    var $li = $("<li>\n                        <div class=\"site\">\n                            <div class=\"logo\">".concat(node.logo[0], "</div>\n                            <div class=\"link\">").concat(simplyUrl(node.url), "</div>\n                            <svg class=\"icon close\">\n                              <use xlink:href=\"#icon-close\"></use>\n                            </svg>\n                        </div>\n                </li>")).insertBefore($lastLi); //设置生成的背景颜色
...
render(); //点击添加按钮之后向hash数组里面添加要添加网页的信息，然后渲染hash数组

$(".addButton").on("click", function () {
  var url = window.prompt("请输入你要添加得网址：");
